...
api: "POST https://api.station.express/api/erc721/defaultMetadata"
description: "Update default metadata for an Badge NFT collection."
---
...
These defaults can be set when [creating a collection](/token-api/badges/post/create) or edited retroactively through this route.
Individual metadata can be set when [minting new NFTs](/token-api/badges/post/mint) or [editing their metadata](/token-api/badges/post/individualMetadata) directly, which will override the defaults set here.
