...
api: "POST https://api.station.express/api/badges/collection/create"
...
<ParamField body="metadataPerToken" type="boolean" optional>
  `true` if metadata will be unique per token or `false` if the same metadata
  applies to all tokens in this collection. Defaults to `true`.
</ParamField>

<ParamField body="metadata" type="object" optional>
    Metadata details if `metadataPerToken` is `false`.
...
<ResponseField name="metadata" type="object">
  Metadata details if token collection is an NFT and `metadataPerToken` is `false`.
...
curl --location --request POST 'https://api.station.express/api/badges/collection/create' \
