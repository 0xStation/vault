...
These defaults can be set when [creating a collection](/token-api/erc721/post/create) or edited retroactively through this route.
Individual metadata can be set when [minting new NFTs](/token-api/erc721/post/mint) or [editing their metadata](/token-api/erc721/post/individualMetadata) directly, which will override the defaults set here.
